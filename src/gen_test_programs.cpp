...
int main()
{
	...
	mem[9] = (ST<<13) | 10<<8 | 13<<4 | 5; // Store the NOT A in F:       ST $TM6, $GP, 5
	...
	mem2[10] = ADD<<13| 1<<12| 9<<8 | 9<<4|0b1111; // Decrement by 1 N--:        ADDI $TM4, $TM4, -1
	...
	mem3[3] = JMP<<13| 1<<12| 6;		             // Go to rec_sum procedure:   JMP 6
	mem3[4] = ST<<13 | 1<<8 | 13<<4| 3;              // Store rec_sum output:      ST $VR, $GP, 3
	...
	mem3[8] = ST<<13 | 15<<8| 14<<4| 0b1111;         // Save the return address:   ST $RA, $SP, -1
	mem3[9] = BR<<13 | 1<<12| 2<<8 | 5<<4 | 3;       // Execute BRANCH if(N>0):    BRMT $AG0, $TM0, 3
	mem3[10] = ADD<<13| 1<<12| 1<<8 | 9<<4| 0;       // Return 0:                  ADDI $VR, $TM0, 0
	mem3[11] = ADD<<13| 1<<12| 14<<8|14<<4| 0b1110;  // Pop 2 items of the stack:  ADDI $SP, $SP, -2
	mem3[12] = JMP<<13| 15<<8;		                 // Return to caller:          JMPR $RA
	mem3[13] = ADD<<13| 1<<12| 2<<8 | 2<<4| 0b1111;  // Else decrement by 1 N--:   ADDI $AG0, $AG0, -1
	mem3[14] = JMP<<13| 1<<12| 6;		             // Go to rec_sum procedure:   JMP 6
	mem3[15] = LD<<13 | 15<<8| 14<<4| 0b1111;        // Restore return address:    LD $RA, $SP, -1
	mem3[16] = LD<<13 | 2<<8 | 14<<4| 0;             // Restore argument N:        LD $AG0, $SP, 0
	mem3[17] = ADD<<13| 1<<12| 14<<8| 14<<4| 0b1110; // Pop 2 items from stack:    ADDI $SP, $SP, -2
	mem3[18] = ADD<<13| 1<<8 | 2<<4 | 1;             // Return N + rec_sum(N-1):   ADD $VR, $AG0, $VR
	...
	mem3[0x103] = 0;                    // Here, the calculated sum will be stored.
	
	// Equivalent C++ procedure for recurrent sum
	//unsigned short rec_sum(unsigned short N);
	//int main()
	//{
	//unsigned short N = 5, result;
	//result = rec_sum(N);
	//return 0;
	//}
	//unsigned short rec_sum(unsigned short N)
